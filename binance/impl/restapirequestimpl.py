...
class RestApiRequestImpl(object):
    ...
    def __create_request_by_delete_with_apikey(self, url, builder):
        request = RestApiRequest()
    ...
    def close_user_data_stream(self, listenKey, accountType):
        check_should_not_none(listenKey, "listenKey")
        builder = UrlParamsBuilder()
        builder.put_url("listenKey", listenKey)

        if(accountType == AccountType.SPOT):
            request = self.__create_request_by_delete_with_apikey("/api/v3/userDataStream", builder)
        elif(accountType == AccountType.MARGIN):
            request = self.__create_request_by_delete_with_apikey("/sapi/v3/userDataStream", builder)
